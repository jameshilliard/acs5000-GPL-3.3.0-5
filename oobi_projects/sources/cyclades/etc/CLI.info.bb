...
[INFO] applications.pm <<<
	- Power Management -
This application allows you to  manage  power outlets  on  Intelligent
Power Distribution Units (IPDU). You can power remote machines on  and
off, cycle the machines, check status, or lock the  power  outlets  so
that they remain in a permanent on or off state.
...
[INFO] config.network.pcmcia <<<
	- Pcmcia Configurations -
Within this feature, you can configure parameters to set up  different
types of PCMCIA cards. The unit supports several types of PCMCIA cards
such as modem, ISDN, wireless and wired NICs, Compact  Flash  and  IDE
drives for data buffer storage.
...
management of the power strips. For  instance,  you  can  set  up  the
association between the servers' power supply and the outlets in which
they are plugged into, or you configure the users who have  access  to
certain outlets.
...
[INFO] config.virtualports <<<
	- Virtual Ports Mode -
An Advanced Console Server can be used as a "master" to  manage  other
Advanced Console Servers (slaves). This idea is called clustering, and
it allows the  stringing  of  several  console  servers  so  that  one
