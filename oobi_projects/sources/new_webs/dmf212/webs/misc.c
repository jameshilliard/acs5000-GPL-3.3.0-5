 ...
 *
 * $Id: misc.c,v 1.2 2006/02/09 22:59:49 livio Exp $
 */
...
static int 	dsnprintf(char_t **s, int size, char_t *fmt, va_list arg,
				int msize);
static int	dstrnlen(char_t *s, unsigned int n);
		...
		int prec, enum flag f)
{
	...
	if (len < 0) { 
		len = dstrnlen(s, prec >= 0 ? prec : ULONG_MAX); 
		...
		int upper, char_t *prefix, int width, int prec, enum flag f)
{
	...
	width -= zeros + len;
	if (prefix != NULL) { 
		width -= dstrnlen(prefix, ULONG_MAX); 
	}
	if (!(f & flag_minus)) {
		if (f & flag_zero) {
			for (i = 0; i < width; ++i) { 
