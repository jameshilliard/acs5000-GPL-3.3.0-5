 ...
 *
 * $Id: default.c,v 1.5 2007/08/20 16:47:51 sergio Exp $
 */
...
#ifdef AVWORKS
const char dscxmlfile[]	= "/discovery.xml";
const char dscuser[]	= "apc";
const char dscpass[]	= "DY3UD2KB532OIXP";
const char dscdtdfile[]	= "/discovery.dtd";
const char imgpath[]	= "/Images/AVWorks/";
#endif


#ifdef AVWORKS
/* Check if the URL is a request for AVWorks files */
static int isAVWorksFile(char_t *url)
{
	if (strcmp(url, dscxmlfile) == 0 ||
	    strcmp(url, dscdtdfile) == 0 ||
	    strncmp(url, imgpath, 16) == 0) {
		return(1);
	}
	else {
		return(0);
...
#ifdef AVWORKS
/* Check user and password for discovery.xml file access */
static int checkDiscoveryPassword(char_t *url)
{
	char user[3], password[15];
	int ret = 0;

	/* if it is a discovery file */
	if (!strncmp(url, dscxmlfile, strlen(dscxmlfile))) {

		/* get user and password */
		strncpy(user, &url[24], sizeof(user));
		strncpy(password, &url[37], sizeof(password));

		/* compare user and password */
		if (!strncmp(user, dscuser, strlen(dscuser)) &&
		    !strncmp(password, dscpass, strlen(dscpass)))
			ret = 0;
		else
			ret = 1;
	}
 ...
 */
...
#ifdef AVWORKS
	/* check user/password for discovery file request */
	if (checkDiscoveryPassword(url)) {
		websError(wp, 403, T(""));
                return 1;
	}

	/* if first access, redirect to a wizard page */
	if (strstr(url, "AVWorks.asp") && stat("/etc/security.opts", &statbuf)) {
		fmtAlloc(&tmp, nchars, T("/wizard/main.asp"));
		websRedirect(wp, tmp);
		bfreeSafe(B_L, tmp);
		return 1;
	}
	...
	/* [EL] We might need a redirect if we should be on an HTTPS page. */
	if ((wp->wsp == NULL) && seclevel) {

#ifdef AVWORKS
		if (!isAVWorksFile(url)) {
#endif
			nchars = gstrlen(path);
			...
			fmtAlloc(&tmp, nchars, T("%s"),websDefaultPage);
			websRedirect(wp,tmp);
			bfreeSafe(B_L, tmp);
			return 1;
#ifdef AVWORKS
 ...
 */
		...
		0666) < 0) 
   {
       ...
       */
		...
		websError(wp, 404, T("Cannot open URL: %s"), path);
		return 1;
	} 
