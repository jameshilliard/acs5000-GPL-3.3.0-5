...
"Usage: dhcpcd [-dknrBCDHNRSTY] [-l leasetime] [-h hostname] [-t timeout]\n\
       [-x mode] [-i vendorClassID] [-I ClientID] [-c filename] [-s [ipaddr]]\n\
  ...
  nleaseTime = htonl(LeaseTime);
	if (DebugFlag) syslog(LOG_DEBUG,"LeaseTime=%d TimeOut=%d",LeaseTime,TimeOut);
  writePidFile(getpid());
  ...
  do
    if ( (currState=(void *(*)())currState()) == NULL ) {
  	if (dhcp_mode != 1) { //[RK]Aug/07/03 Bug#663
  		deletePidFile();
        	exit(1);
	} else {
 		alarm(TimeOut);
  ...
  if (DebugFlag) syslog(LOG_DEBUG,"currState=0 ==> exit 1");
  deletePidFile();
  exit(1);
}
