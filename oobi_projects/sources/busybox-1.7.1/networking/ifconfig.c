...
int ifconfig_main(int argc, char **argv)
{
	...
	char hasBrdCast = 0, gotIfAddr = 0, gotNetMaskAddr = 0;
 ...
 HOSTNAME:
				...
				/* If broadcast was not provided in the command line, set it */
				if (hasBrdCast == 0) {
					/* Keep Interface and Netmask Addresses */
					if (a1op->selector == SIOCSIFADDR) {
						memset(&ifAddr, 0, sizeof(struct sockaddr_in));
						memcpy(&ifAddr, &ifr.ifr_dstaddr, sizeof(struct sockaddr_in));
						gotIfAddr = 1;
					} else if (a1op->selector == SIOCSIFNETMASK) {
						memset(&netMaskAddr, 0, sizeof(struct sockaddr_in));
						memcpy(&netMaskAddr, &ifr.ifr_netmask, sizeof(struct sockaddr_in));
						gotNetMaskAddr = 1;
					}

					/* Set Broadcast Address */
					if (gotIfAddr && gotNetMaskAddr) {
						struct ifreq ifrBrdCast;
						struct sockaddr_in brdCastAddr;

						memset(&ifrBrdCast, 0, sizeof(struct ifreq));
						memset(&brdCastAddr, 0, sizeof(struct sockaddr_in));

						safe_strncpy(ifrBrdCast.ifr_name, ifr.ifr_name, IFNAMSIZ);
						brdCastAddr.sin_family = AF_INET;
						brdCastAddr.sin_port = 0;
						brdCastAddr.sin_addr.s_addr = ifAddr.sin_addr.s_addr | (~netMaskAddr.sin_addr.s_addr);
						memcpy(&(ifrBrdCast.ifr_broadaddr), &brdCastAddr, sizeof(struct sockaddr_in));

						ioctl_or_perror_and_die(sockfd, SIOCSIFBRDADDR, &ifrBrdCast, "%s", "SIOCSIFBRDADDR");
					}
				}
