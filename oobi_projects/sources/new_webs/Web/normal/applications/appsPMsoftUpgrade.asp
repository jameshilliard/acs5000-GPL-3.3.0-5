...
<script type="text/javascript">
	...
	<% 
		...
		var nIpdus = getVal("system.applications.IpduSwUpgrade.numIpdus");
		var swupInfo;
		var latestSW;
		var validIpdus;

		if (nIpdus > 0) {
			swupInfo = getVal("system.applications.IpduSwUpgrade.swupInfoHtml");
			latestSW = getVal("system.applications.IpduSwUpgrade.latest_sw");
			validIpdus = getVal("system.applications.IpduSwUpgrade.nValidIpdus");
		} else {
			swupInfo = "\'none\'";
			latestSW = "none";
			validIpdus = "0";
...
//we fetch all "system.applications.IpduSwUpgrade..." in this page to avoid a second call
//to GetIpduSwUpgrade() while loading appsPMsoftUpgradeMod.asp

var nIpdus = <% write(nIpdus); %>;

if (nIpdus > 0) {

var sysMsg = "<%get("system.applications.IpduSwUpgrade.sysMsg");%>";
var memSign = "<%get("system.applications.IpduSwUpgrade.memSignature");%>";
var swupInfo = <% write(swupInfo); %>;
var latestSW = "<% write(latestSW); %>";
var validIpdus = <% write(validIpdus); %>;

<!-- frames -->
	document.write(
'<frameset  rows="22,*" border="0" frameborder="0">',
	'<frame name="tabs" src="appsPMsoftUpgradeTab.asp?SSID=<%get("_sid_");%>" marginwidth="0" marginheight="0" scrolling="no" frameborder="0" noresize>',
	'<frame name="page" src="appsPMsoftUpgradeMod.asp?SSID=<%get("_sid_");%>" marginwidth="0" marginheight="0" scrolling="auto" frameborder="0" noresize>',
'</frameset>');
	...
	top.helpPage = '/normal/helpPages/appsPMsoftUpgradeHelp.asp';
		...
		<tr>\
			...
			<tr>\
				<td align="center" class="label_for_tables">\
					There is no <%getLabel("system.application.IPDU");%> available to be managed.<br>\
					The <%getLabel("nickname");%> has lost <%getLabel("system.application.IPDU");%> communication.\
				</td>\
			</tr>\
			</table>\
			</td>\
