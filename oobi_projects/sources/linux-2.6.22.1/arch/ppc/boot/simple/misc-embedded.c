...
load_kernel(unsigned long load_addr, int num_words, unsigned long cksum, bd_t *bp)
{
	...
	while (timer++ < 5*1000) {
		...
		if (tstc()) {
			...
			timer = 0;
#ifdef CONFIG_PRxK
			service_wdt();   //[RK]Nov/10/05 - refresh WDT
#endif
		}
		udelay(1000);  /* 1 msec */
	}
	...
	*cp = 0;
	...
	//gunzip(0, 0x300000, zimage_start, &zimage_size);
#ifdef CONFIG_PRxK
	service_wdt();
#endif
