 ...
 *
 * $Id: websSSL.c,v 1.3 2008/01/31 18:03:32 dsueiro Exp $
 */
...
// The webRoot directory will be concatenated with these ones
#define DEFAULT_CERT_FILE   "/etc/CA/server.pem"
#define DEFAULT_KEY_FILE    "/etc/CA/ca-key.pem"
#define DEFAULT_CA_FILE     "/etc/CA/cert.pem"
#define DEFAULT_CA_PATH     "/etc/CA/"
...
char_t			*websSSLHostUrl = NULL;	/* URL to access server */
char_t			*websSSLIpaddrUrl = NULL;	/* URL to access server */
...
int websSSLOpen(int httpsPort)
{
	char		ctFile[128], caPath[128], caFile[128];
	char		*certFile = ctFile, *keyFile = NULL,
...
/*
	sprintf(ctFile, "%s%s", websGetDefaultDir(), DEFAULT_CERT_FILE);
	sprintf(caPath, "%s%s", websGetDefaultDir(), DEFAULT_CA_PATH);
	sprintf(caFile, "%s%s", websGetDefaultDir(), DEFAULT_CA_FILE);
*/
	strcpy(ctFile, DEFAULT_CERT_FILE);
	sprintf(caPath, DEFAULT_CA_PATH);
	sprintf(caFile, DEFAULT_CA_FILE);
 ...
 */
	...
	for(ptr=sslCipher;*ptr;ptr++)
		if(*ptr == ',') *ptr = ' ';
	
	if(!SSL_CTX_set_cipher_list(sslctx,sslCipher)){
		error(E_L, E_USER, T("SSL: Unable to create SSL Cipher List\n"));
               	return -1;
	}
 ...
 */
	if ((!SSL_CTX_load_verify_locations(sslctx, CAfile, CApath)) ||
		...
		error(E_L, E_USER, T("SSL: Unable to set cert verification locations!\n")); 
		websSSLClose();
		return -1;
	}
 ...
 */
	...
	if (httpsPort) {
		...
		if ((sv = getservbyname("https", "tcp")) != NULL) {
			SSL_PORT = sv->s_port;
		}
	}
	...
	if (sslListenSock < 0) {
		error(E_L, E_USER, T("SSL: Unable to open SSL socket on port <%d>!\n"), 
			SSL_PORT);
		syslog(LOG_ALERT, "Https connection rejected. Unable to open SSL socket");
		return -1;
	}

	if (SSL_PORT == 443) {
		websSSLHostUrl = bstrdup(B_L, websHost);
		websSSLIpaddrUrl = bstrdup(B_L, websIpaddr);
	} else {
		fmtAlloc(&websSSLHostUrl, WEBS_MAX_URL + 80, T("%s:%d"), websHost, SSL_PORT);
		fmtAlloc(&websSSLIpaddrUrl, WEBS_MAX_URL + 80, T("%s:%d"),
			websIpaddr, SSL_PORT);

	}
	trace(0, T("webs: Listening for HTTPS requests at address %s\n"),
		websSSLHostUrl);
	return 0;
}
 ...
 *	to the relevant read or write handler. A pointer to the webs structur
...
#ifdef DEV
	ciphers = SSL_get_cipher_list(ssl, 0);
#endif
...
int websSSLSetCertStuff(SSL_CTX *ctx, char *certFile, char *keyFile)
{
	...
	if (certFile != NULL) {
		if (SSL_CTX_use_certificate_file(ctx, certFile, 
			SSL_FILETYPE_PEM) <= 0) {
			error(E_L, E_USER, T("SSL: Unable to set certificate file <%s>\n"),
				certFile); 
			return -1;
		}
		...
		if (SSL_CTX_use_PrivateKey_file(ctx, keyFile, SSL_FILETYPE_PEM) <= 0) {
			error(E_L, E_USER, T("SSL: Unable to set private key file <%s>\n"),
				keyFile); 
			return -1;
		}
 ...
 */
		if (!SSL_CTX_check_private_key(ctx)) {
			error(E_L, E_USER, T("SSL: Check of private key file <%s> FAILED!\n"),
				keyFile); 
			return -1;
		}
	}
 ...
 */
		//[RK]Feb/23/06 - limit the packet's size 
		if (len > 500) {
			return -1;
		}
