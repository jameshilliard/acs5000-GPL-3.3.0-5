...
static char cyc_pass[512];
static struct pam_appl_data cyc_appl_data = {
        id : 0x1234fedc
};

void writeevt(int, const char *, ...);
#endif /* CYCLADES */
...
int login_main(int argc, char **argv)
{
...
#ifdef CYCLADES
	/* log event or not : 0 - NO, 2 = telnet, 4 = console, 5 = dial-in  */
	char logevt = 0;
	struct pam_pwd_ctrl **pwd_ctrl = NULL;
#endif
...
#ifdef CYCLADES
	if (!strncmp(short_tty, "console", 7) || !strncmp(short_tty, "ttyS0", 5))
		logevt = 4;
	else if (!strncmp(short_tty, "ttyM", 4))
		logevt = 5;
#endif
	...
	while (1) {
		...
		cyc_appl_data.ppp    = 0;
		cyc_appl_data.caller = 0;
		cyc_appl_data.port   = 0;
		cyc_appl_data.flag   = 1;
		cyc_appl_data.data   = cyc_pass;
		memset(cyc_pass, 0, sizeof(cyc_pass));
#ifdef GRPAUTHenable
		cyc_appl_data.group_name = NULL;
#endif
		if (opt & LOGIN_OPT_f) {
			pamret = pam_start("adsap2", username, &conv, &pamh);
		} else {
		...
		if (pamret != PAM_SUCCESS) {
			failed_msg = "PAM initialization";
			goto pam_auth_failed;
		}
#ifdef CYCLADES
		pamret = pam_set_item(pamh, PAM_RHOST, fromhost);
		if (pamret != PAM_SUCCESS) {
			failed_msg = "pam_set_item(RHOST)";
			goto pam_auth_failed;
		}
#endif	/* CYCLADES */
		...
		if (pamret == PAM_SUCCESS) {
			...
			pamret = pam_get_item(pamh, PAM_USER, (const void **)(void*)&pamuser);
			if (pamret != PAM_SUCCESS) {
...
#ifdef CYCLADES
			/* [RK]May/30/06 - included the session treatment */
			pamret = pam_setcred(pamh, PAM_ESTABLISH_CRED);
			if (pamret != PAM_SUCCESS) {
				failed_msg = "pam_setcred()";
				goto pam_auth_failed;
			}
			/* open a session for the user --- `0' could be PAM_SILENT */
			pamret = pam_open_session(pamh, 0);
			if (pamret != PAM_SUCCESS) {
				failed_msg = "pam_open_session()";
				goto pam_auth_failed;
			}

			shm_signal(CY_SEM_SHM);
			/* MP[Aug/2004] saved for ts_menu */
			pwd_ctrl = &(cy_shm->pam_passwd);

			while (*pwd_ctrl)
				pwd_ctrl = &((*pwd_ctrl)->next);

			if (!(*pwd_ctrl = (struct pam_pwd_ctrl *)
					shm_malloc(sizeof(struct pam_pwd_ctrl), TYPE_SHM_STR)))
				goto mem_problem;

			memset(*pwd_ctrl, 0, sizeof(struct pam_pwd_ctrl));
			(*pwd_ctrl)->pid = getsid(0);
			strncpy((*pwd_ctrl)->passwd, cyc_pass, sizeof((*pwd_ctrl)->passwd));
#ifdef GRPAUTHenable
			if (cyc_appl_data.group_name) {
				(*pwd_ctrl)->group_name =
					shm_malloc(strlen(cyc_appl_data.group_name) + 1, TYPE_SHM_STR);
				if ((*pwd_ctrl)->group_name) {
					strcpy((*pwd_ctrl)->group_name, cyc_appl_data.group_name);
					free(cyc_appl_data.group_name);
				}
			}
#endif
 mem_problem:
 ...
 auth_failed:
...
#ifdef CYCLADES
	{
		...
		if ((chld_pid = fork()) > 0) {
			...
			if (waitpid(chld_pid, NULL, 0) > 0)
				writeevt(3, "si", username, logevt);		
