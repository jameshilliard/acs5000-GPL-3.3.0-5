...
// workaround to validate iomem between two sockets
ioaddr_t base_used[2] = {0x00,0x00};
			  ...
			  ioaddr_t num, u_int lines)
{
	...
	if ((*base != 0) && 
	    ((*base == base_used[0]) || (*base == base_used[1]))) {
		return 1;
	}
			     ...
			     ioaddr_t num)
{
	...
	for (i = 0; i < MAX_IO_WIN; i++) {
		...
		if ((s->io[i].res->start <= base) &&
			...
			if (s->io[i].InUse == 0) {
				if (s->io[i].res->start == base_used[0]) {
					base_used[0] = 0;
				} else {
					if (s->io[i].res->start == base_used[1]) {
						base_used[1] = 0;
					}
				}
...
/** pcmcia_modify_io
 *
 * Modify I/O parameters specified by req.
 */
int pcmcia_modify_io(struct pcmcia_device *p_dev, io_mod_t *req)
{
	pccard_io_map iomap;
	struct pcmcia_socket *s;
	...
	if(req->Attributes & IO_DATA_PATH_WIDTH_16)
		iomap.flags |= MAP_16BIT;
	if(req->Attributes & IO_DATA_PATH_WIDTH_AUTO)
		iomap.flags |= MAP_AUTOSZ;

	iomap.start = s->io[req->map].res->start;
	iomap.stop = s->io[req->map].res->end;

	s->ops->set_io_map(s, &iomap);
}
EXPORT_SYMBOL(pcmcia_modify_io);
