...
public class Screen6530 implements AsciiCodes {
    ...
    public void cursorToLastCharOnScreen() {
        cursorToLastCharOnRow(visTop + rows - 1, 0);
        if (curCol == (cols - 1)) {
            cursorDown(1);
            doCR();
        }
    ...
    public void cursorToLastCharOnRow() {
        cursorToLastCharOnRow(visTop + curRow, curCol);
    }
    protected void cursorToLastCharOnRow(int row, int col) {
        ...
        if (lastNonBlankCol < col) {
            cursorToLastCharOnRow(row + 1, 0);
            return;
        }
...
foundIt:
    ...
    protected void readLine(StringBuffer buf, int row, int startCol,
                            int endCol) {
