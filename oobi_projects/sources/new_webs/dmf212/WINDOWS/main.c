 ...
 *
 * $Id: main.c,v 1.1.1.1 2005/09/01 02:36:40 edson Exp $
 */
...
#define     kDeviceManagementFramework    T("Device Management Framework")
#define     kAnlCopyright  T("© 2002 Art && Logic, Inc.")
#define     kDmfVersion T("2.1.1")
...
static char_t	*title = T("A&L DMF");/* Window title */
static char_t	*name = T("dmf");			/* Window name */
static char_t	*rootWeb = T("web");			/* Root web directory */
static char_t	*password = T("");				/* Security password */
#ifdef qPort
static int		port = qPort;						/* Server port */
#else
...
static int		bgpTest(int eid, webs_t wp, int argc, char_t **argv);
...
/*
 * Dmf init takes care of all the server setup stuff.
 */
 ...
 */
	...
	if (hSysMenu != NULL)
	{
		...
		AppendMenu(hSysMenu, MF_STRING, IDM_ABOUTBOX, T("About DMF"));
	}
 ...
 */
			...
			if (hwnd) {
				SetWindowText(hwnd, kDmfVersion);
			}
			...
			SetWindowText(hwndDlg, kDeviceManagementFramework);
 ...
 */
	nchar = nCopyAnsiToWideChar(p, "Art && Logic Device Management Framework");
	p += nchar;
 ...
 */
	nchar = nCopyAnsiToWideChar(p, 
		TEXT("Art && Logic Device Management Framework"));
	p += nchar;
 ...
 */
	nchar = nCopyAnsiToWideChar(p, kAnlCopyright);
	p += nchar;
...
static int bgpTest(int eid, webs_t wp, int argc, char_t **argv)
{
   ...
   s = bstrdup(B_L, name);
	retval =  websWrite(wp, T("bgpTest: %s -> %s"), name, strupper(s));
   bfree(B_L, s);
...
static void memLeaks() 
{
	...
	if ((fd = gopen(T("leak.txt"), O_CREAT | O_TRUNC | _O_RDWR)) >= 0) {
