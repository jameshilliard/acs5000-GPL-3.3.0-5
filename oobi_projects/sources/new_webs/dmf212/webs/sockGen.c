 ...
 *
 * $Id: sockGen.c,v 1.2 2007/09/21 12:54:21 giuliano Exp $
 */
 ...
 */
	...
	// [GM]Sep/20/07- This code enclosed in if 0 is not being ported to IPv6 because it is not used in broadcast mode for Cyclades applications.
#if 0
 ...
 */
	...
	//[GM]Sept/20/07 - IPv6 support added
	//try ipv6 at first
	sp->sock = socket(AF_INET6, dgram ? SOCK_DGRAM: SOCK_STREAM, 0);
	if (sp->sock < 0) { // if ipv6 failed try ipv4 only
		sp->sock = socket(AF_INET, dgram ? SOCK_DGRAM: SOCK_STREAM, 0);
		if (sp->sock < 0) { // ip ipv4 socket failed to open return an error condition
			socketFree(sid);
			return -1;
		}
		else {
			hasIPv6 = 0; // ipv6 not available 
		}
	}
	else {
		hasIPv6 = 1;
 ...
 */
	memset((char *) &addr_storage, '\0', sizeof(addr_storage));
	if (hasIPv6) {
		sockaddr6 = (struct sockaddr_in6 *)&addr_storage;
		sockaddr6->sin6_family = AF_INET6;
		sockaddr6->sin6_addr = in6addr_any;
		sockaddr6->sin6_port = htons((short) (port & 0xFFFF));
	}
	else {
		sockaddr4 = (struct sockaddr_in *)&addr_storage;
		sockaddr4->sin_family = AF_INET;
		sockaddr4->sin_addr.s_addr = INADDR_ANY;
		sockaddr4->sin_port = htons((short) (port & 0xFFFF));
	}
 ...
 */
		...
		if (hasIPv6) {
			int errorNo=0;
			errorNo = bind(sp->sock, (struct sockaddr *)&addr_storage,sizeof(*sockaddr6));
			if (errorNo < 0) {
				socketFree(sid);
				return -1;
			}
			...
			if (bind(sp->sock, (struct sockaddr *) &addr_storage,sizeof(*sockaddr4)) < 0) {
				socketFree(sid);
				return -1;
			}
		}
