...
#ifdef CYCLADES
	// password is not allowed for appliance session
	if ((thisport < 0) &&
...
//[RK]Sep/30/05 - do not accept the first authenticatication (without password)
#ifndef CYCLADES
...
#else // undef CYCLADES
	if ((thisport >= 0) && PRIVSEP(auth_password(authctxt, ""))) {
#ifdef USE_PAM
		if (options.use_pam && (PRIVSEP(do_pam_account())))
#endif
		{
			//[RK]May/10/06 auth_log(authctxt, 1, "without authentication", "");
			return;
		}
	}
#endif
...
#endif
#ifdef CYCLADES
		if ((thisport >= 0) && (authenticated ==1)) { // access serial port
			//[RK]Jun/02/05 - Group Authorization Support
			if (!cyc_check_access(authctxt->pw->pw_name,getppid())) {
				//deny 
...
do_authentication(Authctxt *authctxt)
{
...
#ifdef CYCLADES
	if (style) {
		if (cyc_check_term_config(style)) {
			packet_disconnect("Remote server SSHD disconnect");
		}
	} else {
		if (cyc_check_local_ip()) {
			packet_disconnect("Remote server SSHD disconnect");
		}
	}
	
	if (use_privsep && thisport >= 0) mm_register_thisport(thisport,user);

	if (thisport >=0) cyc_copy_user_pai(user);
#endif
	...
	if ((authctxt->pw = PRIVSEP(getpwnamallow(user))) != NULL) {
		authctxt->valid = 1;
#ifdef CYCLADES
		strcpy(authctxt->pw->pw_name, user); //[RK]May/27/04
#endif
