...
#ifdef CONFIG_TSxK
static inline void CycSetFpgaIntr(struct serial_state *state, int st);
#endif
...
static int TTYunavailable = 0; //mp: May, 2003 - improve ISR loop

#if defined(MODULE) && defined(SERIAL_DEBUG_MCOUNT)
#define DBG_CNT(s) printk("(%s): [%x] refc=%d -> %s\n", \
		kdevname(tty->device),			\
		(info->flags), tty->refcount, s)
#else
#define DBG_CNT(s)
#endif
...
#define BOARD_TYPE ((bd_t *)__res)->hw_info.board_type
#define IS_BOARD_KVM (BOARD_TYPE == BOARD_KVM16 || BOARD_TYPE == BOARD_KVM32 ||\
	BOARD_TYPE == BOARD_KVMNET16 || BOARD_TYPE == BOARD_KVMNET32)

//[RK]Aug/10/05 - reinserted the paranoia check
#define SERIAL_PARANOIA_CHECK
static inline int serial_paranoia_check(struct async_struct *info, const char *routine)
{
#ifdef SERIAL_PARANOIA_CHECK
	static const char *badmagic =
		"Warning: bad magic number for serial struct in %s\n";
	static const char *badinfo =
		"Warning: null async_struct for in %s\n";

	if (!info) {
		printk(badinfo, routine);
		return 1;
	}
	if (info->magic != SERIAL_MAGIC) {
		printk(badmagic,routine);
		return 1;
	}
#endif
	return 0;
...
static void rs_stop(struct tty_struct *tty)
{
	...
	if (serial_paranoia_check(info, "rs_stop"))
		return;
...
static void rs_start(struct tty_struct *tty)
{
	...
	if (serial_paranoia_check(info, "rs_start"))
		return;
...
static _INLINE_ void receive_chars(struct async_struct *info, int *status)
{
	...
	do {
#if 0
		/**
		 *   \todo Check if we can remove this. If so, remove also the
		 *   variable #TTYunavailable.
		 *   Be careful to not to return without settting *status.
		 */
...
#endif
		ch = inb(info->port + UART_RX);
		flag = TTY_NORMAL;
		icount->rx++;

#ifdef SERIAL_DEBUG_INTR
		printk("DR%02x:%02x...", ch, lsr);
#endif
...
#endif
		/* >> From kernel 2.6.22.1 - serial_core.h [RGB] */
		if ((lsr & info->ignore_status_mask & ~UART_LSR_OE) == 0)
		 ...
		 */
		if (lsr & ~info->ignore_status_mask & UART_LSR_OE)
			tty_insert_flip_char(tty, 0, TTY_OVERRUN);
		/* << from kernel 2.6.22.1 - serial_core.h */
...
static _INLINE_ void transmit_chars(struct async_struct *info, int *intr_done)
{
...
#ifdef CONFIG_PRxK	//[RK]
	if ((info->xmit.head == info->xmit.tail) &&
	    (info->flags & ASYNC_LEGACY_RTS)) {
		count = serial_inp(info,UART_LSR);
		if (!(count & UART_LSR_TEMT)) {
			info->IER |= UART_IER_THRI;
			serial_out(info,UART_IER,info->IER);
			return;
		}
	...
	if (info->xmit.head == info->xmit.tail
		...
		if (info->flags & ASYNC_LEGACY_RTS) {
			if (!(info->flags & ASYNC_CTS_FLOW) || (!info->tty->hw_stopped)) { //[mp]Jul/2004
...
#ifdef SERIAL_DEBUG_INTR
	printk("THRE %d ...",info->xmit_fifo_size - count);	//[RK]
#endif
	if (intr_done)
		*intr_done = 0;
...
static _INLINE_ void check_modem_status(struct async_struct *info)
{
	...
	if ((info->flags & ASYNC_CHECK_CD) && (status & UART_MSR_DDCD)) {
		...
		if (status & UART_MSR_DCD)
		{
			...
			printk(KERN_ALERT "PORT DCD: Port %d DCD went high\n",info->line);
			wake_up_interruptible(&info->open_wait);
		} else {
			//[RK]Aug/02/04 - SNMPTRAP for DCD
			printk(KERN_ALERT "PORT DCD: Port %d DCD went low\n",info->line);
			//[RK]Jun/25/05 - back port from 2.4
			if (!((info->flags & ASYNC_CALLOUT_ACTIVE) &&
			      (info->flags & ASYNC_CALLOUT_NOHUP))) {
			//if ((info->flags & ASYNC_CALLOUT_NOHUP)) 
...
static irqreturn_t rs_interrupt(int irq, void *dev_id)
{
...
#ifdef SERIAL_DEBUG_INTR
		printk("line %d status = %x...", info->line, status);	//[RK]
#endif
	...
	next:
		...
		if (!info) {
			info = IRQ_ports[irq];
			if (TTYunavailable || (pass_counter++ > RS_ISR_PASS_LIMIT)) {
				TTYunavailable = 0;
#if 0
 ...
 */
static irqreturn_t rs_interrupt_multi(int irq, void *dev_id)
{
	int status;
	struct async_struct * info;
	int pass_counter = 0;
...
static void do_softint(struct work_struct *private_)
{
	...
	struct async_struct *info =
		container_of(private_, struct async_struct, work);
	...
	if ((jiffies - last_strobe) >= RS_STROBE_TIME) {
		for (i=0; i < NR_IRQS; i++) {
			...
			if (info->next_port) {
				...
				{	//[RK]
#ifdef SERIAL_DEBUG_INTR
					printk("rs_timer call rs_interrupt \n");
#endif
					rs_interrupt(i, NULL);
				}
			} else 
			...
			{	//[RK]
#ifdef SERIAL_DEBUG_INTR
				printk("rs_timer call rs_interrupt_single \n");
#endif
...
static int startup(struct async_struct * info)
{
...
#ifdef SERIAL_DEBUG_OPEN
	printk("starting up ttys%d type %d (irq %d)...", info->line, state->type, state->irq); //[RK]
#endif
	...
	if (IRQ_VALID(state->irq) && (!IRQ_ports[state->irq] ||
		...
		retval = request_irq(state->irq, handler, IRQF_SHARED,
	...
	case BOARD_KVMNET32:
		if (info->line >= 5) {
			muxfpga->uart_int_2 |= (1 << (info->line - 5));	
			break;
		}
	case BOARD_KVM16:
	case BOARD_KVM32:
		if (info->line >= 1) {
			muxfpga->uart_int |= (1 << (info->line - 1));
		}
		break;
	}
#endif
...
static void shutdown(struct async_struct * info)
{
	...
	/* the serial ports need of the following commands  [RK]Dec/06/04 */
...
static int baud_table[] = {
	...
	600, 1200, 1800, 2400, 4800, 9600, 14400, 19200,
	28800, 38400, 57600, 76800, 115200, 230400, 460800, 0 };
...
static void change_speed(struct async_struct *info, struct ktermios *old)
{
	...
	unsigned cflag, iflag, cval, fcr = 0;  //[RK]
	int	bits;
	unsigned long	flags;
	...
	if (!quot && old) {
		...
		info->tty->termios->c_cflag |= (old->c_cflag & CBAUD);
		baud = tty_get_baud_rate(info->tty);
		if (!baud)
			baud = 9600;
		if (baud == 38400 &&
	...
	printk(KERN_DEBUG "ttyS%d: '%s' selected baud rate of %d (div=%d)\n",
	       info->state->line + SERIAL_DEV_OFFSET, current->comm, baud, quot);
	...
	if (uart_config[info->state->type].flags & UART_USE_FIFO) {
		if ((info->state->baud_base / quot) < 2400) // speed < 2400
			fcr = UART_FCR_ENABLE_FIFO | UART_FCR_TRIGGER_1; //  8-RX 8-TX
#ifdef CONFIG_SERIAL_RSA
		else if (info->state->type == PORT_RSA)
			fcr = UART_FCR_ENABLE_FIFO | UART_FCR_TRIGGER_14;
#endif
		else
			fcr = UART_FCR_ENABLE_FIFO | UART_FCR_TRIGGER_8; // 56-RX 8-TX
	}
	if (info->state->type == PORT_16750)
...
#define RELEVANT_IFLAG(iflag) (iflag & (IGNBRK|BRKINT|IGNPAR|PARMRK|INPCK))
	...
	local_irq_save(flags);
	if (uart_config[info->state->type].flags & UART_STARTECH) { //[RK]
		unsigned save_efr = ((cflag & CRTSCTS) ? (UART_EFR_RTS|UART_EFR_CTS) : 0);
		/* XON/XOFF flow control flags */
		iflag = info->tty->termios->c_iflag;
		if (iflag & IXON) save_efr |= 0x0a;
		serial_outp(info, UART_LCR, 0xBF);
		serial_outp(info, UART_EFR, UART_EFR_ECB);
		serial_outp(info, UART_EFR, save_efr);
		serial_outp(info, UART_XON1, 0x11);
		serial_outp(info, UART_XOFF1, 0x13);
	}
...
static void rs_put_char(struct tty_struct *tty, unsigned char ch)
{
	...
	if (serial_paranoia_check(info, "rs_put_char"))
		return;
...
static void rs_flush_chars(struct tty_struct *tty)
{
	...
	if (serial_paranoia_check(info, "rs_flush_chars"))
		return;
	...
	serial_out(info, UART_IER, info->IER);
	if (pxa_port(info->state->type))
		rs_interrupt_single(info->state->irq, NULL);
	local_irq_restore(flags);
}
...
static int rs_write(struct tty_struct * tty, const unsigned char *buf, int count)
{
	...
	if (serial_paranoia_check(info, "rs_write"))
		return 0;
...
#ifdef CONFIG_PRxK
		if (info->flags & ASYNC_LEGACY_RTS &&
			(!tty->hw_stopped || !(info->flags & ASYNC_LEGACY_CTS))) { //[mp]Jul/2004
			info->MCR |= UART_MCR_RTS; // RTS ON
			serial_out(info,UART_MCR,info->MCR);
		}	
#endif
		if (!tty->hw_stopped) { //[mp]Jul/2004
			info->IER |= UART_IER_THRI;
			serial_out(info, UART_IER, info->IER);
			if (pxa_port(info->state->type)) {
				local_irq_save(flags);
...
static int rs_write_room(struct tty_struct *tty)
{
	...
	if (serial_paranoia_check(info, "rs_write_room"))
		return 0;
...
static int rs_chars_in_buffer(struct tty_struct *tty)
{
	...
	if (serial_paranoia_check(info, "rs_chars_in_buffer"))
		return 0;
...
static void rs_flush_buffer(struct tty_struct *tty)
{
	...
	if (serial_paranoia_check(info, "rs_flush_buffer"))
		return;
	local_irq_save(flags);
	info->xmit.head = info->xmit.tail = 0;
	local_irq_restore(flags);
	...
	/* Clear the FIFO buffers and disable them */
	if (uart_config[info->state->type].flags & UART_CLEAR_FIFO) { //[RK]
		serial_outp(info, UART_FCR,
			(UART_FCR_CLEAR_RCVR | UART_FCR_CLEAR_XMIT | UART_FCR_ENABLE_FIFO));
	}
...
static void rs_send_xchar(struct tty_struct *tty, char ch)
{
	...
	if (serial_paranoia_check(info, "rs_send_char"))
		return;
...
static void rs_throttle(struct tty_struct * tty)
{
	...
	if (serial_paranoia_check(info, "rs_throttle"))
		return;
	
	//[RK]Jan/13/06 - problem with PCMCIA
...
static void rs_unthrottle(struct tty_struct * tty)
{
	...
	if (serial_paranoia_check(info, "rs_unthrottle"))
		return;
...
static int get_modem_info(struct tty_struct *tty, struct file *file)
{
	struct async_struct * info = (struct async_struct *)tty->driver_data;
	...
	//[RK]Aug/11/05 - problem with PCMCIA	
	if (serial_paranoia_check(info, "set_modem_info"))
		return 0;
...
static int set_modem_info(struct tty_struct *tty, struct file *file,
...
static void rs_break(struct tty_struct *tty, int break_state)
{
	...
	if (serial_paranoia_check(info, "rs_break"))
		return;
...
#ifdef CONFIG_PRxK	//[RK]Jul/23/03 - Cyclades IOCTL command
static int cyc_set_command(struct async_struct *info, unsigned int * arg)
{
	unsigned long flags;
	...
	if ((command & 0xffff0000) == CYC_GET_MEDIA) {
		// get media value 
		media = (info->flags & ASYNC_LEGACY_RTS) ? 4 : 0;
		media = (info->flags & ASYNC_LEGACY_CTS) ? 8 : 0;
	...
	if ((command & 0xffff0000) == CYC_SET_MEDIA) {
		...
		if ((media & 0xa) == 0xa) { // RTS/CTS Half Duplex 
			if (info->flags & ASYNC_CTS_FLOW) { // HW flow control
				return -EINVAL;
			}
			info->flags |= ASYNC_LEGACY_CTS | ASYNC_LEGACY_RTS;
		} else {
			if (media & 2) { // Half Duplex 
//[mp]Jul/2004
//				if (info->flags & ASYNC_CTS_FLOW) { // HW flow control
//					return -EINVAL;
//				}
		...
		if (info->flags & ASYNC_LEGACY_RTS) {
			// do RTS down and set Modem Interr.
			info->MCR &= ~UART_MCR_RTS;
			local_irq_save(flags);
			serial_out(info, UART_MCR, info->MCR);
			local_irq_restore(flags);
			...
			//mp: shouldn't serial_out(info, UART_IER, info->IER) always
			// be called to define a condition?
			// If HALF Duplex and no CTS control
			// UART_IER will not be programmed !!!
			}
		} else { // Full 
			// do RTS UP and reset Modem Interr.
			info->MCR |= UART_MCR_RTS;
			if (!(info->flags & (ASYNC_CHECK_CD|ASYNC_CTS_FLOW))) {
			    	// not HW flow control nor local
				info->IER &= ~UART_IER_MSI;
			}
			local_irq_save(flags);
			serial_out(info, UART_MCR, info->MCR);
		    ...
		    unsigned int cmd, unsigned long arg)
{
	...
	if (serial_paranoia_check(info, "rs_ioctl"))
		return -ENODEV;
...
#ifndef CONFIG_PRxK	//[RK]Feb/11/05 - kernel 2.6
		case TIOCMGET:
...
static void rs_set_termios(struct tty_struct *tty, struct ktermios *old)
{
	struct async_struct *info = (struct async_struct *)tty->driver_data;
	unsigned long flags;
...
static void rs_close(struct tty_struct *tty, struct file * filp)
{
	...
	if (!info || serial_paranoia_check(info, "rs_close"))
		return;
...
static void rs_wait_until_sent(struct tty_struct *tty, int timeout)
{
	...
	if (serial_paranoia_check(info, "rs_wait_until_sent"))
		return;
...
static void rs_hangup(struct tty_struct *tty)
{
	...
	if (serial_paranoia_check(info, "rs_hangup"))
		return;
...
//		MOD_DEC_USE_COUNT;
	...
	info->tty = tty;
	if (serial_paranoia_check(info, "rs_open"))
		return -ENODEV;
		 ...
		 int *eof, void *data)
{
	...
	len += sprintf(page, "Cpld serial info:1.0 driver:%s%s revision:%s\n",
...
#endif
printk("\ndetect_uart_irq port=%lu", state->port);
...
printk("\ndetect_uart_irq port=%lu ret-irq=%d",state->port, irq);
...
int old_register_serial(struct serial_struct *req, struct device *device);
void old_unregister_serial(int line);

#if (LINUX_VERSION_CODE > 0x20100)
EXPORT_SYMBOL(old_register_serial);
EXPORT_SYMBOL(old_unregister_serial);
#else
...
#endif
		line = old_register_serial(&serial_req, NULL);
		if (line < 0)
			break;
...
#ifdef CONFIG_PRxK  
	/*
	 * The tty_io calls specifics routines for the following ioctls:
	 *	TIOCMGET, TIOCMSET, TIOCMBIC and TIOCMBIS
	 */
	.tiocmget = get_modem_info, /* TIOCMGET */
	.tiocmset = set_modem_info, /* TIOCMSET, TIOCMBIC and TIOCMBIS */
#endif
};
...
static int __init rs_init(void)
{
	...
	serial_driver->driver_name = "serial_ttyS";
	serial_driver->name = "ttyS";
	serial_driver->name_base = SERIAL_DEV_OFFSET;
	serial_driver->major = CYCLADES_MAJOR;
	...
	for (i = 0, state = rs_table; i < NR_PORTS; i++,state++) {
		...
		if (state->io_type == SERIAL_IO_MEM) {
			printk(KERN_INFO"ttyS%02d%s at 0x%p (irq = %d) is a %s\n",
			       ...
			       state->iomem_base, state->irq/2,
			       uart_config[state->type].name);
		} else {
			printk(KERN_INFO "ttyS%02d%s at 0x%04lx (irq = %d) is a %s\n",
	 		       state->line + SERIAL_DEV_OFFSET,
			       (state->flags & ASYNC_FOURPORT) ? " FourPort" : "",
			       state->port, state->irq/2,
			       uart_config[state->type].name);
		}
	}
#ifdef ENABLE_SERIAL_PCI
...
/**
 *	old_register_serial - configure a 16x50 serial port at runtime
...
int old_register_serial(struct serial_struct *req, struct device *device)
{
	...
	state->custom_divisor = req->custom_divisor;
	if (req->baud_base)
	...
	tty_register_device(serial_driver, state->line, device);

	return state->line;
}

/**
 *	old_unregister_serial - deconfigure a 16x50 serial port
...
#ifdef CONFIG_TSxK
static void display_cpld(char *x)
{
	...
	case BOARD_KVMNET16:
	case BOARD_KVMNET32:
		// print for KVM should be put here
		break;
	default:
		printk("CPLD-%x %s: m=%x r1=%x ck1=%x sc1=%x "
			"i1=%x i2=%x i3=%x i4=%x s1=%x s2=%x s3=%x s4=%x\n",
			cpld->fpga_id, x, cpld->cpld_misc, cpld->reset_enable1,
			cpld->clock_enable1,
			cpld->clock_source1, cpld->intr_enable1, cpld->intr_enable2,
			cpld->intr_enable3, cpld->intr_enable4, cpld->intr_status1,
			cpld->intr_status2, cpld->intr_status3, cpld->intr_status4);
		if (CYC_BOARD_TYPE == BOARD_TS3K || CYC_BOARD_TYPE == BOARD_ACS48 || CYC_BOARD_TYPE == BOARD_AVCS48 || CYC_BOARD_TYPE == BOARD_ACS5048) {
			printk("  r2=%x ck2=%x sc2=%x i5=%x i6=%x s5=%x s6=%x\n",
				cpld->reset_enable2, cpld->clock_enable2, 
				cpld->clock_source2, cpld->intr_enable5, 
				cpld->intr_enable6, cpld->intr_status5, 
				cpld->intr_status6);
		}
	}
}
...
static inline void CycSetFpgaIntr(struct serial_state *state, int st)
{
	...
	if (st)
		*state->mux |= state->mux_bit;
	else
		*state->mux &= ~state->mux_bit;
}

void tsxk_detect_board(void)
{
	int i;
	volatile UART_REGS *sexr = uarts;
//	volatile UART_REGS *sexr_db9 = uarts_db9;
	struct serial_state * state = &rs_table[1];
	bd_t    *bd = (bd_t *)__res;

	// disable interrupts in all ports
	switch (bd->hw_info.board_type) {
	case BOARD_KVM16:
	case BOARD_KVM32:
	case BOARD_KVMNET16:
	case BOARD_KVMNET32:
		muxfpga->uart_int_2 = 0x00;
		muxfpga->uart_int   = 0x00;
		break;
	case BOARD_ONS441:
	case BOARD_ONS481:
	case BOARD_ONS841:
	...
	case BOARD_ONS882:
		muxfpga->uart_int_2 = 0x00;
		muxfpga->uart_int   = 0x00;
		cpld->intr_enable1  = 0x00;
		break;
	default:
		cpld->intr_enable1  = 0x00;
		cpld->intr_enable2  = 0x00;
		cpld->intr_enable3  = 0x00;
		cpld->intr_enable4  = 0x00;
		cpld->intr_enable5  = 0x00;
		cpld->intr_enable6  = 0x00;
		break;
	}
	...
	case BOARD_TS3K: // TS3000
		nports = 48;
		cpld->reset_enable1 = 0xff;
		cpld->clock_source1 = 0x00;
		cpld->clock_enable1 = 0xff;
		cpld->reset_enable2 = 0x0f;
		cpld->clock_source2 = 0x00;
		cpld->clock_enable2 = 0x0f;
		break;

	case BOARD_ACS48: // ACS48
	case BOARD_AVCS48: // AVCS48
	case BOARD_ACS5048: // ACS5048
		nports = 48;
		cpld->reset_enable1 = 0x03;
		cpld->reset_enable2 = 0x01;
		if (cpld->fpga_id == 0x32) {
			/* buggy CPLD - must run on 14.7 MHz */
			cpld->clock_source1 = 0xff;
			cpld->clock_source2 = 0x0f;
		} else {
			cpld->clock_source1 = 0x00;
			cpld->clock_source2 = 0x00;
		}
	...
	case BOARD_AVCS32: // AVCS32
	case BOARD_ACS5032: // ACS5032
		nports = 32;
		cpld->reset_enable1 = 0x03;
		cpld->clock_source1 = 0x00;
		cpld->clock_enable1 = 0xff;
	...
	case BOARD_TS1K:// TS1000
		nports = 16;
		cpld->reset_enable1 = 0x0f;
		cpld->clock_source1 = 0x00;
		cpld->clock_enable1 = 0x0f;
		break;

	case BOARD_TS1100:// TS1100
	case BOARD_AVCS16: // AVCS16
	case BOARD_ACS5016: // ACS5016
		nports = 16;
		cpld->reset_enable1 = 0x01;
		cpld->clock_source1 = 0x00;
		cpld->clock_enable1 = 0x0f;
		break;
	...
	case BOARD_TS8H:// TS800
		...
		cpld->reset_enable1 = 0x03;
		cpld->clock_source1 = 0x00;
		cpld->clock_enable1 = 0x03;
		break;

	case BOARD_ACS8:// ACS8
	case BOARD_AVCS8:// AVCS8
	case BOARD_ACS5008:// ACS5008
		nports = 8;
		cpld->reset_enable1 = 0x03;
		cpld->clock_source1 = 0x00;
		cpld->clock_enable1 = 0x03;
	...
	case BOARD_TS4H:// TS400
		...
		cpld->reset_enable1 = 0x01;
		cpld->clock_source1 = 0x00;
		cpld->clock_enable1 = 0x01;
		break;

	case BOARD_ACS4:// ACS4
	case BOARD_AVCS4:// AVCS4
	case BOARD_ACS5004:// ACS5004
		nports = 4;
		cpld->reset_enable1 = 0x01;
		cpld->clock_source1 = 0x00;
		cpld->clock_enable1 = 0x01;
	...
	case BOARD_AVCS1:// AVCS1
	case BOARD_ACS5001:// ACS5001
		state->count = 1;
		return;
	case BOARD_KVM16:
	case BOARD_KVM32:
		...
		state->hub6 = 0;
		nports = 5;
		muxfpga->misc |= 4;	/* Reset enable */
		muxfpga->misc &= ~2;	/* UART clock source (busclk/2) */
		muxfpga->misc |= 1;	/* clock enable */
		break;
	case BOARD_KVMNET16:
	case BOARD_KVMNET32:
		state->count = 1;
		...
		muxfpga->misc &= ~2;	/* UART clock source (busclk/2) */
		muxfpga->misc |= 1;	/* clock enable */
		muxfpga->misc_2 |= 4;	/* Reset enable */
		muxfpga->misc_2 &= ~2;	/* UART clock source (busclk/2) */
		muxfpga->misc_2 |= 1;	/* clock enable */
		break;
	case BOARD_ONS441:
	case BOARD_ONS481:
	case BOARD_ONS442:
	...
	if (IS_BOARD_KVM) {
		muxfpga->misc &= ~4;
		if (bd->hw_info.board_type == BOARD_KVMNET16 ||
			bd->hw_info.board_type == BOARD_KVMNET32) {
			muxfpga->misc_2 &= ~4;
		}
	} else {
		cpld->reset_enable1 = 0x00;

		if (bd->hw_info.board_type == BOARD_TS3K || 
			bd->hw_info.board_type == BOARD_ACS48 || 
			bd->hw_info.board_type == BOARD_AVCS48 ||
			bd->hw_info.board_type == BOARD_ACS5048) {
			cpld->reset_enable2 = 0x00;
		}
	}

	for (i=1; i <= nports && i < NR_PORTS; i++, state++) {
		if (state->port || (state->hub6 & 0x10000) == 0) {
			continue;
		}
		...
		switch (bd->hw_info.board_type) {
			...
			case BOARD_ONS882:
				if (i < 6) {
					state->mux = &muxfpga->uart_int;
					state->mux_bit = 1 << (i - 2);
					break;
				}
				if (i < 10) {
					state->mux = &muxfpga->uart_int_2;
					state->mux_bit = 1 << (i - 6);
					break;
				}
				if (i < 18) {
					state->mux = &cpld->intr_enable1;
					state->mux_bit = 1 << (i - 10);
					break;
				}
				break;
			case BOARD_KVMNET16:
			...
			default:
				...
				if (i <= 16) {
					state->mux = &cpld->intr_enable2;
					state->mux_bit = 1 << (i - 9);
					break;
				}
				if (i <= 24) {
					state->mux = &cpld->intr_enable3;
					state->mux_bit = 1 << (i - 17);
					break;
				}
				if (i <= 32) {
					state->mux = &cpld->intr_enable4;
					state->mux_bit = 1 << (i - 25);
					break;
				}
				if (i <= 40) {
					state->mux = &cpld->intr_enable5;
					state->mux_bit = 1 << (i - 33);
					break;
				}
				state->mux = &cpld->intr_enable6;
				state->mux_bit = 1 << (i - 41);
				break;
		}
		...
		switch (bd->hw_info.board_type) {
			...
			case BOARD_AVCS48:
			case BOARD_ACS5048:
				state->port = (int)&sexr[i-1] - _IO_BASE;
				if (cpld->fpga_id != 0x32) {
					state->baud_base /= 2;
				}
				if (i < 17) { /* 1 - 16 */
			...
			case BOARD_ACS5032:
				state->baud_base /= 2;
				state->irq = (i > 16) ? SIU_IRQ2 : SIU_IRQ1;
				state->port = (int)&sexr[i-1] - _IO_BASE;
				break;
			case BOARD_KVM16:
			case BOARD_KVM32:
			...
			case BOARD_KVMNET32:
				state->irq = SIU_IRQ4;
				state->port = (int)&sexr[i-2] - _IO_BASE;
				break;
			case BOARD_ONS441:
			case BOARD_ONS481:
			case BOARD_ONS841:
			...
			case BOARD_ONS882:
				state->irq = SIU_IRQ4;
				if (i > 9) {
//					state->port = (int)&sexr_db9[i-10] - _IO_BASE;
				} else {
					state->port = (int)&sexr[i-2] - _IO_BASE;
				}
				break;
			default: 
				state->irq = (i > 32) ? SIU_IRQ2 : SIU_IRQ1;
				state->port = (int)&sexr[i-1] - _IO_BASE;
				break;
		}
		state->flags = CYCLADES_FLAGS;
	}
	display_cpld("init");
}
#endif
