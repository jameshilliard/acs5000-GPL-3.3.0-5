...
static int setup_dma_config(struct pcmcia_device *link, int start_pg,
			    int stop_pg);
#endif
static int setup_config(struct pcmcia_device *link, int start_pg,
			int stop_pg);
...
static void set_io_16(struct net_device *dev, int set16)
{
	io_mod_t req;

	req.map = 0;
	if (set16)
		req.Attributes = IO_DATA_PATH_WIDTH_16;
	else
		req.Attributes = 0;

	pcmcia_modify_io(PRIV(dev)->p_dev, &req);
}
...
static int pcnet_config(struct pcmcia_device *link)
{
	...
	setup_config(link, start_pg, stop_pg);
...
static void pcnet_reset_8390(struct net_device *dev)
{
    ...
    outb_p(E8390_NODMA+E8390_PAGE0+E8390_STOP, nic_base + E8390_CMD); /** \todo Need this? */
...
static void ei_watchdog(u_long arg)
{
	...
	printk(KERN_INFO "%s: %s link beat\n", dev->name,
        ...
        /* Necessary for the bonding driver */
        if (link)
