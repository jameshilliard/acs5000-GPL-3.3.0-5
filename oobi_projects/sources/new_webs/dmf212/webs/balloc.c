 ...
 *
 * $Id: balloc.c,v 1.1.1.1 2005/09/01 02:36:40 edson Exp $
 */
 ...
 */
		if (bFlags & B_USE_MALLOC) {
			...
			if (bp == NULL) {
				error(E_L, E_USER, T("B: malloc failed\n"));
				return NULL;
			}
#ifdef B_STATS
...
void bstats1(int handle, void (*writefn)(int handle, char_t *fmt, ...))
{
	bType			*bp;
...
void bstats2(int handle, void (*writefn)(int handle, char_t *fmt, ...))
{
 ...
 */
	...
	total = 0;
	(*writefn)(handle, 
		T("  bytes, blocks in use, total times, largest,   q, File\n"));

	for (fp = files; fp < &files[bStatsFilesMax]; fp++) {
		if (fp->file[0]) {
			(*writefn)(handle, T("%7d,         %5d,      %6d, %7d,%4d, %s\n"),
				fp->allocated, fp->count, fp->times, fp->largest, 
				fp->q, fp->file);
			total += fp->allocated;
		}
	}
...
void bstats3(int handle, void (*writefn)(int handle, char_t *fmt, ...))
{
	char_t			*cp;
	bStatsBlkType	*blkp;
	bStatsFileType	*fp;
...
void bstats(int handle, void (*writefn)(int handle, char_t *fmt, ...))
{
	...
	bstats1(handle, writefn);
	bstats2(handle, writefn);
	bstats3(handle, writefn);
