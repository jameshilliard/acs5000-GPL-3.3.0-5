...
/* prints DSView/AVWorks plugin version number */
static int plugin_read_proc(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	bd_t *bd = (bd_t *)__res;
	int len;
		...
		int count, int *eof, void *data)
{
	...
	sprintf(page,
		...
		pdev,
		bd->hw_info.fpga_version, bd->hw_info.board_version,
		bd->hw_info.ipboard1,
		bd->hw_info.ipboard2,
		pmac,
		CYC_CF_FS,
		k26,
		bd->cyc_serial_number[0],bd->cyc_serial_number[1],bd->cyc_serial_number[2],bd->cyc_serial_number[3],bd->cyc_serial_number[4]
		);
	len = strlen(page);
	return proc_calc_metrics(page, start, off, count, eof, len);
}
...
#ifdef CONFIG_PRxK
	if (bd->hw_info.board_type < BOARD_AVCS1 || bd->hw_info.board_type > BOARD_AVCS48) {
#ifndef OEM3
#define PLUGIN "dsview-plug-in"
#else
#define PLUGIN "avworks-plug-in"
#endif
		/* creates /proc entry with DSView/AVWorks plugin version number */
		create_proc_read_entry(PLUGIN, 0, NULL, plugin_read_proc, NULL);
	}
#endif
