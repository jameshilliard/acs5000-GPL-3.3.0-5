...
#ifndef OEM3
/* No OEM: use the first parameter. */
#define OEM_SELECT(_name1, _name2) _name1
#else
/* OEM3: use the second parameter. */
#define OEM_SELECT(_name1, _name2) _name2
#endif
...
static void reset_ports(int board)
{
	...
	if (board == BOARD_ACS1 || board == BOARD_AVCS1 || 
		board == BOARD_ACS5001 ||
		board == BOARD_KVM16 || board == BOARD_KVM32) {
...
embed_config(bd_t **bdp)
{
	...
	*bdp = bd;
	...
	pflash = FLASH_ADDR + CNF_ADDRESS1;
	...
	switch (bd->hw_info.wdt_config) {
		...
		case 1:
			bd->wdt = 1;
			break;
		default: //[RK]Jun/24 - back port from 2.4 kernel
			bd->wdt = 0;
			break;
	}
	...
	if (!bd->bi_enetaddr[1] && !bd->bi_enetaddr[2]) {
		...
		bd->bi_enetaddr[1] = 0xe0;
		bd->bi_enetaddr[2] = 0x86;
		bd->bi_enetaddr[3] = 0x00;
		bd->bi_enetaddr[4] = 0x00;
		bd->bi_enetaddr[5] = 0x01;
	}

	//[RK]Jul/01/10 - Ananke - Appliance Serial Number
	if (p_om) {
		memcpy(bd->cyc_serial_number,p_om->serial_number,5);
	} else {
		memcpy(bd->cyc_serial_number,"00000",5);
	}

	//[RK]Dec/07/05 -- CYCLADES_VERSION ==> max size 47 bytes (defined in the bd struct)
	bd->cyc_version[47] = 0x00;
	for (i=0 ; i < 47 ; i++) {
		bd->cyc_version[i] = *(CYCLADES_VERSION + i);
		if (bd->cyc_version [i] == NULL) break;
	}

	//[RK]Jun/07/06 - DSVIEW Plug-in version
#ifndef AVCS
	_bcopy_str(CYCLADES_DSVIEW_PLUGIN, bd->dsview_plugin);
#endif
	...
	} else if (p->board_type == BOARD_KVMNET16 || 
		p->board_type == BOARD_KVMNET32) {
		_bcopy_str(PROD_LINE5, bd->prod_line);
		_bcopy_str(PROD_NAME5, bd->prod_name);
	} else if (p->board_type == BOARD_ONS441 || 
		p->board_type == BOARD_ONS481 || 
		p->board_type == BOARD_ONS881 || 
		p->board_type == BOARD_ONS882 || 
		p->board_type == BOARD_ONS442 || 
		p->board_type == BOARD_ONS482 || 
		p->board_type == BOARD_ONS841 || 
		p->board_type == BOARD_ONS842 ) {
		_bcopy_str(PROD_LINE6, bd->prod_line);
		_bcopy_str(PROD_NAME6, bd->prod_name);
	} else if (p->board_type == BOARD_SM100) {
		_bcopy_str(PROD_LINE2, bd->prod_line);
		_bcopy_str(PROD_NAME2, bd->prod_name);
#ifdef AVCS
	} else if (p->board_type == BOARD_AVCS1 || 
		p->board_type == BOARD_AVCS4 || 
		p->board_type == BOARD_AVCS8 || 
		p->board_type == BOARD_AVCS16 || 
		p->board_type == BOARD_AVCS32 || 
		p->board_type == BOARD_AVCS48) {
		_bcopy_str(PROD_LINE8, bd->prod_line);
		_bcopy_str(PROD_NAME8, bd->prod_name);
#endif
	} else if (p->board_type == BOARD_ACS5001 || 
		p->board_type == BOARD_ACS5004 || 
		p->board_type == BOARD_ACS5008 || 
		p->board_type == BOARD_ACS5016 || 
		p->board_type == BOARD_ACS5032 || 
		p->board_type == BOARD_ACS5048) {
		_bcopy_str(PROD_LINE9, bd->prod_line);
		_bcopy_str(PROD_NAME9, bd->prod_name);
	} else {
		_bcopy_str(PROD_LINE0, bd->prod_line);
		_bcopy_str(PROD_NAME0, bd->prod_name);
	}
	_bcopy_str(AVOCENT_TXT, bd->vendor_name);
	bd->num_portsS = 0;
	bd->num_portsA = 0;
	bd->num_portsK = 0;
	bd->num_portsM = 0;
	...
	case BOARD_ACS48:		// ACS48
		_bcopy_str(OEM_SELECT("ACS", "CPS"), bd->card_name);
		bd->num_portsS = 48;
		bd->num_portsM = 2;
		break;
	...
	case BOARD_ACS8:		// ACS8
		...
		bd->num_portsM = 2;
		break;
	case BOARD_ONS441:		//ONS441
	case BOARD_ONS442:
		_bcopy_str("OnSite", bd->card_name);
		bd->num_portsS = 4;
		bd->num_portsK = 4;
		bd->num_portsA = 3;
		bd->num_portsM = 2;
	...
	case BOARD_KVM16:
	case BOARD_KVM32:
		_bcopy_str("KVM", bd->card_name);
		bd->num_portsA = 1;
		break;
	case BOARD_KVMNET16:
	case BOARD_KVMNET32:
		_bcopy_str("KVM/NET", bd->card_name);
		bd->num_portsA = 1;
	...
	case BOARD_KVMP16:
	case BOARD_KVMP32:
		_bcopy_str("KVM/PLUS", bd->card_name);
		bd->num_portsA = 2;
		bd->num_portsK = 16;
		bd->num_portsM = 2;
		break;
	case BOARD_AVCS1:		// AVCS1
		_bcopy_str("AVCS", bd->card_name);
		bd->num_portsS = 1;
		break;
	...
	case BOARD_ACS5001:		// ACS5001
		_bcopy_str("ACS5000", bd->card_name);
		bd->num_portsS = 1;
		break;
