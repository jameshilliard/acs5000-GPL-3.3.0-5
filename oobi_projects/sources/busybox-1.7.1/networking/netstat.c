...
static void build_ipv6_addr(const char* local_addr, struct sockaddr_in6* localaddr)
{
...
#if ENABLE_FEATURE_IPV6
static void build_ipv4_addr(const char* local_addr, struct sockaddr_in6* localaddr)
#else
static void build_ipv4_addr(const char* local_addr, struct sockaddr_in* localaddr)
#endif
{
	memset(localaddr, 0, sizeof(*localaddr));
	sscanf(local_addr, "%X",
