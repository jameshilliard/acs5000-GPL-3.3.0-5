...
extern	time_t		resolv_init_mtime;
extern	time_t		yp_init_mtime;
extern	time_t		ntp_init_mtime;
...
{
  register int i;
  if (DebugFlag) syslog(LOG_DEBUG,"releaseDhcpOptions\n");
  for (i=1;i<256;i++)
...
void (*buildUdpIpMsg)(unsigned);
{
  ...
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpSendAndRecv xid=%x msg=%x\n",xid,msg);
  ...
  do
	...
	if (DebugFlag) syslog(LOG_DEBUG,"dhcpSendAndRecv - received one msg\n");
      do
	{
	  ...
	  if ( parseDhcpMsgRecv() == msg ){
		if (DebugFlag) syslog(LOG_DEBUG,"dhcpSendAndRecv - ret 0 = MSG received OK\n");
		 return 0;
	  }
	  if ( DhcpOptions.val[dhcpMessageType] )
...
void *dhcpStart()
{
  ...
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpStart\n");
  ...
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpStart == ret dhcpInit\n");
  return &dhcpInit;
}
/*****************************************************************************/
...
void *dhcpReboot()
{
  if ( sigsetjmp(env,0xffff) )
    {
      if ( DebugFlag )
	syslog(LOG_DEBUG,"dhcpReboot - timed out waiting for DHCP_ACK response\n");
      alarm(TimeOut);
      return &dhcpInit;
    }
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpReboot\n");
  dhcpStart();
  memset(&DhcpOptions,0,sizeof(DhcpOptions));
...
#endif
      classclientsetup();
      if (DebugFlag) syslog(LOG_DEBUG,"dhcpReboot - readDhcpCache - ret dhcpInit\n");
      return &dhcpInit;
    }
  classclientsetup();
#if 0
  if ( ClientMACaddr_ind )
    ...
    memcpy(DhcpIface.chaddr,ClientHwAddr,ETH_ALEN);
#endif
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpReboot - dhcpRequest random(Reboot)\n");
...
DhcpIface.client_id[6],DhcpIface.client_id[7],DhcpIface.client_id[8]);
  ...
  if ( DebugFlag ) syslog(LOG_DEBUG,"dhcpInit - broadcasting DHCP_DISCOVER\n");
  if ( dhcpSendAndRecv(random(),DHCP_OFFER,&buildDhcpDiscover) )
    {
      if (DebugFlag) syslog(LOG_DEBUG,"dhcpInit - dhcpStop\n");
      dhcpStop();
      return 0;
    }
...
/* DHCP_OFFER received */
  ...
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpInit - dhcpRequest xid=%x Request\n",DhcpIface.xid);
...
/* send the message and read and parse replies into DhcpOptions */
  if (DebugFlag)
    syslog(LOG_DEBUG,"dhcpRequest : broadcasting DHCP_REQUEST for %u.%u.%u.%u\n",
  ...
  if ( dhcpSendAndRecv(xid,DHCP_ACK,buildDhcpMsg) ) {
  	if (DebugFlag) syslog(LOG_DEBUG,"dhcpRequest xid=%x dhcpInit\n",xid);
	 return &dhcpInit;
  }
  ReqSentTime=time(NULL);
  if ( DebugFlag ) syslog(LOG_DEBUG,
    "dhcpRequest : DHCP_ACK received from %s (%u.%u.%u.%u)\n",DhcpMsgRecv->sname,
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[0],
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[1],
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[2],
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[3]);
...
/* check if the offered IP address already in use */
  if ( arpCheck() )
    {
      if ( DebugFlag ) syslog(LOG_DEBUG,
	"dhcpRequest : requested %u.%u.%u.%u address is in use\n",
	((unsigned char *)&DhcpIface.ciaddr)[0],
	((unsigned char *)&DhcpIface.ciaddr)[1],
	((unsigned char *)&DhcpIface.ciaddr)[2],
	((unsigned char *)&DhcpIface.ciaddr)[3]);
  ...
  if ( DebugFlag ) syslog(LOG_DEBUG,
    "dhcpRequest : verified %u.%u.%u.%u address is not in use\n",
    ((unsigned char *)&DhcpIface.ciaddr)[0],
    ((unsigned char *)&DhcpIface.ciaddr)[1],
    ((unsigned char *)&DhcpIface.ciaddr)[2],
    ((unsigned char *)&DhcpIface.ciaddr)[3]);
  ...
  if ( dhcpConfig() )
    {
      if (DebugFlag) syslog(LOG_DEBUG,"dhcpRequest after dhcpConfig dhcpStop\n");
      dhcpStop();
      return 0;
    }
  ...
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpRequest OK dhcpBound\n");
  return &dhcpBound;
}
/*****************************************************************************/
void *dhcpBound()
{
  int i;
  if ( sigsetjmp(env,0xffff) ){
         if (DebugFlag) syslog(LOG_DEBUG,"dhcpBound == SIGALRM == dhcpRenew\n");
	 return &dhcpRenew;
  }
  i=ReqSentTime+ntohl(*(unsigned int *)DhcpOptions.val[dhcpT1value])-time(NULL);
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpBound time=%d T1=%d\n",i,
  	ntohl(*(u_int *)DhcpOptions.val[dhcpT1value]));
  if ( i > 0 )
    alarm(i);
  else
    return &dhcpRenew;

  if (DebugFlag) syslog(LOG_DEBUG,"dhcpBound sleep T1=%d\n",
  	ntohl(*(u_int *)DhcpOptions.val[dhcpT1value]));
  sleep(ntohl(*(u_int *)DhcpOptions.val[dhcpT1value]));
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpBound => dhcpRenew\n");
  return &dhcpRenew;
}
/*****************************************************************************/
void *dhcpRenew()
{
  int i;
  if ( sigsetjmp(env,0xffff) ) {
        if (DebugFlag) syslog(LOG_DEBUG,"dhcpRenew SIGALRM ==> dhcpRebind\n");
	  return &dhcpRebind;
  }
  i = ReqSentTime+ntohl(*(unsigned int *)DhcpOptions.val[dhcpT2value])-time(NULL);
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpRenew time=%d T2=%d\n",i,
  	ntohl(*(u_int *)DhcpOptions.val[dhcpT2value]));
  if ( i > 0 )
    alarm(i);
  else
    return &dhcpRebind;
  ...
  if ( DebugFlag )
    syslog(LOG_DEBUG,"dhcpRenew sending DHCP_REQUEST for %u.%u.%u.%u to %u.%u.%u.%u\n",
	   ((unsigned char *)&DhcpIface.ciaddr)[0],
	   ((unsigned char *)&DhcpIface.ciaddr)[1],
	   ((unsigned char *)&DhcpIface.ciaddr)[2],
	   ((unsigned char *)&DhcpIface.ciaddr)[3],
  ...
  ReqSentTime=time(NULL);
  if ( DebugFlag ) syslog(LOG_DEBUG,
    "dhcpRenew DHCP_ACK received from %s (%u.%u.%u.%u) --> dhcpBound\n",DhcpMsgRecv->sname,
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[0],
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[1],
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[2],
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[3]);
...
void *dhcpRebind()
{
  int i;
  if ( sigsetjmp(env,0xffff) ) {
        if (DebugFlag) syslog(LOG_DEBUG,"dhcpRebind SIGALRM ==> dhcpStop\n");
	  return &dhcpStop;
  }
  i = ReqSentTime+ntohl(*(unsigned int *)DhcpOptions.val[dhcpIPaddrLeaseTime])-time(NULL);
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpBound time=%d Lease=%d\n",i,
  	ntohl(*(u_int *)DhcpOptions.val[dhcpIPaddrLeaseTime]));
  if ( i > 0 )
    alarm(i);
  else
    return &dhcpStop;
  ...
  if ( DebugFlag )
    syslog(LOG_DEBUG,"dhpcRebind broadcasting DHCP_REQUEST for %u.%u.%u.%u\n",
	   ((unsigned char *)&DhcpIface.ciaddr)[0],
	   ((unsigned char *)&DhcpIface.ciaddr)[1],
	   ((unsigned char *)&DhcpIface.ciaddr)[2],
	   ((unsigned char *)&DhcpIface.ciaddr)[3]);
  ...
  ReqSentTime=time(NULL);
  if ( DebugFlag ) syslog(LOG_DEBUG,
    "dhcpRebind DHCP_ACK received from %s (%u.%u.%u.%u) ==> dhcpBound\n",DhcpMsgRecv->sname,
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[0],
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[1],
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[2],
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[3]);
...
{
  struct sockaddr addr;
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpRelease\n");
  deleteDhcpCache();
  if ( DhcpIface.ciaddr == 0 ) {
          if (DebugFlag) syslog(LOG_DEBUG,"dhcpReleasei ciaddr==0 dhcpInit\n");
	  return &dhcpInit;
  }
  ...
  memcpy(addr.sa_data,IfName,IfName_len);
  if ( DebugFlag )
    syslog(LOG_DEBUG,"dhcpRelease sending DHCP_RELEASE for %u.%u.%u.%u to %u.%u.%u.%u\n",
	   ((unsigned char *)&DhcpIface.ciaddr)[0],
	   ((unsigned char *)&DhcpIface.ciaddr)[1],
	   ((unsigned char *)&DhcpIface.ciaddr)[2],
	   ((unsigned char *)&DhcpIface.ciaddr)[3],
  ...
  DhcpIface.ciaddr=0;
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpRelease set ciaddr=0 dhcpInit\n");
  return &dhcpInit;
}
/*****************************************************************************/
static time_t stat_mtime(const char *fname)
{
  struct stat stbuf;
  if ( stat(fname, &stbuf) )
    return 0;
...
void *dhcpStop()
{
  ...
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpStop\n");
  releaseDhcpOptions();
...
#endif
#ifdef IPv6enable
  /* Leave the interface up so the IPv6 addresses are not lost. */
...
tsc:
  close(dhcpSocket);
  /* Replace the files only if they weren't touched by anyone else. */
  if ( resolv_renamed && resolv_init_mtime == stat_mtime(RESOLV_CONF) )
    rename(""RESOLV_CONF".sv",RESOLV_CONF);
  if ( yp_renamed && yp_init_mtime == stat_mtime(NIS_CONF) )
    rename(""NIS_CONF".sv",NIS_CONF);
  if ( ntp_renamed && ntp_init_mtime == stat_mtime(NTP_CONF) )
...
void *dhcpInform()
{
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpInform\n");
  dhcpStart();
  memset(&DhcpOptions,0,sizeof(DhcpOptions));
  memset(&DhcpIface,0,sizeof(dhcpInterface));
  ...
  if ( sigsetjmp(env,0xffff) )
    {
      if ( DebugFlag )
	syslog(LOG_DEBUG,"dhcpInform : timed out waiting for DHCP_ACK response\n");
      return 0;
    }
  if ( DebugFlag )
    syslog(LOG_DEBUG,"dhcpInform : broadcasting DHCP_INFORM for %u.%u.%u.%u\n",
	   ((unsigned char *)&DhcpIface.ciaddr)[0],
	   ((unsigned char *)&DhcpIface.ciaddr)[1],
	   ((unsigned char *)&DhcpIface.ciaddr)[2],
	   ((unsigned char *)&DhcpIface.ciaddr)[3]);
  ...
  if ( DebugFlag ) syslog(LOG_DEBUG,
    "dhcpInform : DHCP_ACK received from %s (%u.%u.%u.%u)\n",DhcpMsgRecv->sname,
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[0],
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[1],
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[2],
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[3]);
...
/* check if the offered IP address already in use */
  if ( arpCheck() )
    {
      if ( DebugFlag ) syslog(LOG_DEBUG,
	"dhcpInform : requested %u.%u.%u.%u address is in use\n",
	((unsigned char *)&DhcpIface.ciaddr)[0],
	((unsigned char *)&DhcpIface.ciaddr)[1],
	((unsigned char *)&DhcpIface.ciaddr)[2],
	((unsigned char *)&DhcpIface.ciaddr)[3]);
  ...
  if ( DebugFlag ) syslog(LOG_DEBUG,
    "dhcpInform : verified %u.%u.%u.%u address is not in use\n",
    ((unsigned char *)&DhcpIface.ciaddr)[0],
    ((unsigned char *)&DhcpIface.ciaddr)[1],
    ((unsigned char *)&DhcpIface.ciaddr)[2],
    ((unsigned char *)&DhcpIface.ciaddr)[3]);
#endif
  if ( dhcpConfig() ) {
  	if (DebugFlag) syslog(LOG_DEBUG,"dhcpInform : dhcpConfig --> ret 0\n");
	  return 0;
  }
  if (DebugFlag) syslog(LOG_DEBUG,"dhcpInform : dhcpConfig(ret 0) -> exit 0\n");
  exit(0);
}
