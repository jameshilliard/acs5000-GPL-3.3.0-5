...
#define RCSID	"$Id: eap.c,v 1.2 2008/10/29 00:08:18 edson Exp $"
	...
	case eapIdentify:
		...
		if (get_srp_secret(esp->es_unit, esp->es_server.ea_peer,
			...
			if ((cp2 = strchr(cp, ':')) == NULL)
				break;
			*cp2++ = '\0';
			tpw = t_newpw(); //[RK]Jan/24/06
			tpw->pebuf.name = esp->es_server.ea_peer;
			//[RK]Jan/24/06
			tpw->pebuf.password.len = t_cstrfromb64(tpw->pwbuf, cp);
