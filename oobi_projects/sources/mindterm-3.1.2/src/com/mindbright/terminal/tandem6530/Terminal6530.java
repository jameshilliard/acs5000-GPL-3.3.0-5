...
public class Terminal6530
    ...
    public Terminal6530() {
        ...
        blockMode = new BlockMode(this, MAX_PAGES);
    ...
    public void doReadWithAddress(int startRow, int startCol,
                                  int endRow, int endColumn) {
        debug("doReadWithAddress");
        send(modes[currentMode].doReadWithAddress(startRow, startCol,
                endRow, endColumn));
    }
    ...
    public void doReadWithAddressAll(int startRow, int startCol,
        ...
        send(modes[currentMode].doReadWithAddressAll(startRow, startCol,
                endRow, endColumn));
    }
    ...
    public void doResetMoifiedDataTags() {
        debug("doResetMoifiedDataTags");
        modes[currentMode].doResetMoifiedDataTags();
    }
    public void doReadWholePageOrBuffer() {
        debug("doReadWholePageOrBuffer");
        send(modes[currentMode].doReadWholePageOrBuffer());
    }
        ...
        if (termWin != null && str != null) {
            ...
            if (DEBUG) {
                ...
                for (int i = 0; i < ca.length; i++) {
                    ...
                    buf.append(" (");
                    buf.append(Integer.toHexString((int) ca[i]));
                    buf.append(")\n");
                }
                debug(buf.toString());
            }
    ...
    private boolean haveAltShift(int modifiers) {
        return haveModifiers(modifiers,
