...
#ifdef CYC
#include "mibgroup/cyacsmgmt/cystrings.h"
#endif

#ifdef CYC
oid      version_sysoid[SIZE_PROD_VERSION_SYSOID];
int      version_sysoid_len = SIZE_PROD_VERSION_SYSOID;
#else
...
init_master_agent(void)
{
    ...
    if (cptr) {
	    ...
	    if ((int)cy_shm == -1) {
		if (shm_init(-1) < 0) {
		    snmp_log(LOG_ERR, "problems initializing shared memomy - cy_shm\n");
		} else {
		    snmp_log(LOG_DEBUG, "will wait for the shared memory semaphore...\n");
		    while (shm_get_set_val(0, &cy_shm->st_cy_ras, 0, CY_SEM_CONFIG) != 1) {
			snmp_log(LOG_DEBUG, "wait\n");
			sleep(1);
		    }
		    snmp_log(LOG_DEBUG, "shared memory is ready\n");
		}    
	    }
#endif
        /*
...
#ifdef IPv6enable
	    if (cy_shm->main_conf.enableIPv6) {
		sprintf(buf, "tcp6:%d", SNMP_PORT);
	    } else {
