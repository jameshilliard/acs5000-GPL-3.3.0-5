...
struct serial_quirk {
	...
	void (*setup)(struct pcmcia_device *, struct serial_struct *);
...
static void quirk_config_globetrotter(struct pcmcia_device *link, struct serial_struct *serial)
{
	/* [RGB] BUG#4399
	 *
	 * GlobeTrotter works only with a custom divisor of 1.  Here we set
	 * the custom divisor to 1, but its value is used only when the
	 * user-selected speed of the serial port is 38400. In any other case
	 * the divisor will be baud_base/<baud_rate>. If the quotient is 0 the
	 * the port defaults to a baud rate of 9600 (see serial.c). So, in
	 * order to work correctly we need a baud_rate of 9600 or 38400.
	 */
	printk(KERN_NOTICE "GlobeTrotter card detected: speed must be 9600 or 38400\n");
...
static const struct serial_quirk quirks[] = {
	{
		...
		.prodid	= ~0,
		.multi	= -1,
		.setup	= quirk_config_globetrotter,
	}, {
		.manfid	= MANFID_IBM,
		.prodid	= ~0,
		.multi	= -1,
...
static void serial_remove(struct pcmcia_device *link)
{
	...
	for (i = 0; i < info->ndev; i++)
		old_unregister_serial(info->line[i]);
...
static int serial_suspend(struct pcmcia_device *link)
{
	...
	/** \todo FIX THIS: Enter/leave suspended mode. */
	for (i = 0; i < info->ndev; i++)
			...
			kio_addr_t iobase, int irq)
{
	...
	if (info->quirk && info->quirk->setup)
		info->quirk->setup(handle, &serial);

	line = old_register_serial(&serial, &handle_to_dev(handle));
	if (line < 0) {
	...
	sprintf(info->node[info->ndev].dev_name, "ttyM%d", line - 48);
