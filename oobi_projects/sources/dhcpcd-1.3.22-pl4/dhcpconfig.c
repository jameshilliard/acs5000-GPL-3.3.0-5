...
time_t	resolv_init_mtime = 0;
time_t	yp_init_mtime = 0;
time_t	ntp_init_mtime = 0;
  ...
  if ( fork() == 0 )
#endif
    {
      char *argc[6],exec_on_change[128],mode[4];
      if ( Cfilename )
	...
	snprintf(exec_on_change,sizeof(exec_on_change),EXEC_ON_CHANGE,ConfigDir);
      ...
      if ( DebugFlag )
        ...
        argc[4]=NULL;
      argc[5]=NULL;
      if (DebugFlag) syslog(LOG_DEBUG,"execute_on_change [%s %s %s %s]\n",exec_on_change,hostinfo_file,prm,mode);
      if ( execve(exec_on_change,argc,ProgramEnviron) && errno != ENOENT )
	syslog(LOG_ERR,"error executing \"%s %s %s %s\": %m\n",
	exec_on_change,hostinfo_file,prm,mode);
      exit(0);
    }
}
...
int dhcpConfig()
{
...
#endif
	  fclose(f);
	  if (stat(RESOLV_CONF, &stbuf) == 0)
		  resolv_init_mtime = stbuf.st_mtime;
	}
      else
  ...
  if ( ReplNISConf )
      ...
      if ( f )
	{
	  ...
	  fclose(f);
	  if (stat(NIS_CONF, &stbuf) == 0)
		  yp_init_mtime = stbuf.st_mtime;
	}
      else
	syslog(LOG_ERR,"dhcpConfig: fopen: %m\n");
    }
  if ( ReplNTPConf )
    {
      ...
      if ( f )
 	{
 	  ...
 	  fclose(f);
	  if (stat(NTP_CONF, &stbuf) == 0)
		  ntp_init_mtime = stbuf.st_mtime;
...
IfNameExt,
DhcpIface.class_id);
if (DhcpMsgRecv->file[0]) {
	fprintf(f,"BOOT_FILE=\'%s\'\n", DhcpMsgRecv->file);
}
      if ( ClientID )
