 ...
 * Schneier's (who is the inventor of the algorithm) c-code found in his book:
...
public final class Blowfish extends BlockCipher {
    ...
    throws InvalidKeyException {
        ...
        for(i = 0; i < 16 + 2; i += 2) {
            ...
            P[i]     = getIntMSBO(LR, 0);
            P[i + 1] = getIntMSBO(LR, 4);
        }

        for(j = 0; j < 256; j += 2) {
            blockEncrypt(LR, 0, LR, 0);
            S0[j]     = getIntMSBO(LR, 0);
            S0[j + 1] = getIntMSBO(LR, 4);
        }
        for(j = 0; j < 256; j += 2) {
            blockEncrypt(LR, 0, LR, 0);
            S1[j]     = getIntMSBO(LR, 0);
            S1[j + 1] = getIntMSBO(LR, 4);
        }
        for(j = 0; j < 256; j += 2) {
            blockEncrypt(LR, 0, LR, 0);
            S2[j]     = getIntMSBO(LR, 0);
            S2[j + 1] = getIntMSBO(LR, 4);
        }
        for(j = 0; j < 256; j += 2) {
            blockEncrypt(LR, 0, LR, 0);
            S3[j]     = getIntMSBO(LR, 0);
            S3[j + 1] = getIntMSBO(LR, 4);
        }
    }
