...
/* straight out of RFC-1305 Appendix A */
	...
	double etime,vstime,skew1,skew2;
	int freq;
...
#undef Data
	...
	if (set_clock) {   /* you'd better be root, or ntpclient will crash! */
		...
		//[RK] BUg1216 - set hte hardware clock
		if (stime(&tv_set.tv_sec) < 0) {
			perror("stime");
		}
	...
	vstime=ntpdiff(&rectime,&xmttime);
	...
	printf("%d %5d.%.3d  %8.1f %8.1f  %8.1f %8.1f %9d\n",
		...
		arrival->fine/4294967, etime, vstime,
		(skew1-skew2)/2, sec2u(disp), freq);
	fflush(stdout);
}

void setup_receive(int usd, char *port)
{
	struct addrinfo *ai;
	...
	if (getaddrinfo (NULL, port, &hints, &ai) != 0) {
        ...
        // bind socket
	if(bind(usd, (struct sockaddr *)ai->ai_addr, ai->ai_addrlen) == -1) {
		fprintf(stderr,"could not bind to udp port %s\n", port);
		perror("bind");
		exit(1);
	}
...
void setup_transmit(int usd, struct addrinfo *ai)
{
	unsigned char i;

	// try 10 times until it is able to connect.
	for (i = 0; i < 10; i ++) {
		if (connect(usd, (struct sockaddr *)ai->ai_addr,
			ai->ai_addrlen) !=-1) {
			return;
		}
		perror("connect");
...
void do_replay(void)
{
	...
	while (fgets(line,sizeof(line),stdin)) {
		n=sscanf(line,"%d %f %f %f %lf %f %d",
			&day, &sec, &etime, &vstime, &skew, &disp, &freq);
		if (n==7) {
			fputs(line,stdout);
...
int main(int argc, char *argv[]) {
	...
	char *hostname = NULL;        /* must be set */
        char *udp_local_port = NULL;  /* default of 0 means kernel chooses */
	...
	hints.ai_socktype = SOCK_DGRAM;
	hints.ai_protocol = IPPROTO_UDP;
	if (getaddrinfo (hostname, NTP_PORT, &hints, &ai) != 0) {
		perror ("getaddrinfo");
		exit(1);
	}

	// get socket for communication
	if ((usd = socket(ai->ai_family, ai->ai_socktype,
		ai->ai_protocol)) == -1) {
		perror ("socket");
		exit(1);
	}
	...
	// setup reception
	if (udp_local_port != NULL)
		setup_receive(usd, udp_local_port);

        // setup transmission
	setup_transmit(usd, ai);
