...
#ifdef CYCLADES
#define INITTAB_PCMCIA "/etc/inittab_pcmcia"  /* inittab file location */

/* Create a flag to indicate that action was read from inittab_pcmcia vs. inittab */
#define FROM_INITTAB_PCMCIA 1

/*
 *  Modified version of parse_inittab() & new_init_action() having an
 *  additional parameter.  We also define a macro for those calls to
 *  the normal versions of these functions.
 */
static void parse_inittab_ex(int which_inittab);
#define parse_inittab() \
	parse_inittab_ex(0)

static void new_init_action_ex(int action, const char *command,
	const char *cons, int which_inittab);
#define new_init_action(_action, _command, _cons) \
	new_init_action_ex(_action, _command, _cons, 0)

#else
#define parse_inittab_ex(_which) \
	parse_inittab()
#define new_init_action_ex(_action, _command, _cons, _which)	\
	new_init_action(_action, _command, _cons)
#endif
...
#ifdef CYCLADES
static void new_init_action_ex(int action, const char *command, const char *cons, int which_inittab)
#else
...
#ifdef CYCLADES
	new_action->which_inittab = which_inittab;
#endif
...
#ifdef CYCLADES   /* [RK]Jan/30/03 Cyclades */
static void parse_timezone(void)
{
	FILE *file;
	char tzvalue[256], *p;

	if ((file = fopen("/etc/TIMEZONE", "r")) != NULL) {
		if (fgets(tzvalue, 255, file) != NULL) {
			if ((p = strchr(tzvalue, '\n'))) *p = 0;
			if ((p = strchr(tzvalue, '\r'))) *p = 0;
			setenv("TZ", tzvalue, 1);
		}
		fclose(file);
	}
...
#ifdef CYCLADES
	if (which_inittab == FROM_INITTAB_PCMCIA) {
		file = fopen(INITTAB_PCMCIA, "r");
	} else {
	...
	while (fgets(buf, INIT_BUFFS_SIZE, file) != NULL) {
		...
		for (a = actions; a->name != 0; a++) {
			if (strcmp(a->name, action) == 0) {
				...
				new_init_action_ex(a->action, command, id, which_inittab);
				break;
			}
		}
		if (a->name == 0) {
...
#ifdef CYCLADES
	parse_inittab_ex(FROM_INITTAB_PCMCIA);
#endif
...
int init_main(int argc, char **argv)
{
...
#ifdef CYCLADES /* [RK]Dec/29/04 */
	{
		struct init_action cyc_init;

		strcpy(cyc_init.command, "/bin/init_ram");
		strcpy(cyc_init.terminal, "");
		cyc_init.action = SYSINIT;
		cyc_init.pid = 0;
		cyc_init.next = 0;
		waitfor(&cyc_init, 0);
		parse_timezone();
	...
	if (argc > 1
	 && (!strcmp(argv[1], "single") || !strcmp(argv[1], "-s") || LONE_CHAR(argv[1], '1'))
		...
		{
			struct init_action cyc_init;
			strcpy(cyc_init.command, "/bin/cy_shm");
			strcpy(cyc_init.terminal, "");
			cyc_init.action = SYSINIT;
			cyc_init.pid = 0;
			cyc_init.next = 0;
		...
		new_init_action(RESPAWN, bb_default_login_shell, "/dev/ttyS0");
		new_init_action(RESPAWN, "/bin/cy_wdt_led wdt led", "/dev/null");
#else
