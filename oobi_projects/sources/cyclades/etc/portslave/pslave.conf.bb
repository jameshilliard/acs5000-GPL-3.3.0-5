...
#
#  NOTE: In order to use Modbus in LS1001A, uncomment the related line in /etc/inetd.conf and
...
# When 0, data are always buffered whether or not there is a session 
# to the port sending data to the unit.
# When 1, data are NOT buffered when there IS a session to the port 
# sending data to the unit, but resumes the buffering of data when 
# there ISN'T a session to the port.
#
...
# manufacturer. We currently support IPDU from BB (bb).
# The configuration file of IPDU is /etc/pm.bb file.
#
#all.pmtype  bb
