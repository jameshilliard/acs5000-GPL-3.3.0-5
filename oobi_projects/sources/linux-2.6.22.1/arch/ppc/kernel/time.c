...
void timer_interrupt(struct pt_regs * regs)
{
	...
	while ((next_dec = tb_ticks_per_jiffy - tb_delta(&jiffy_stamp)) <= 0) {
		 ...
		 */
#ifdef CONFIG_PRxK
//[RK]Jun/28/05 - back port from 2.4 kernel
		//[RK]Nov/04/05 - update the rtc after 1 sec
		if (!last_rtc_update || ((xtime.tv_sec - last_rtc_update) >= 659)) {
			ppc_md.set_rtc_time(xtime.tv_sec+1 + timezone_offset);
