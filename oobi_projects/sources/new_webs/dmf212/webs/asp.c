 ...
 *
 * $Id: asp.c,v 1.2 2006/01/10 23:51:53 erik Exp $
 */
...
int checkWebErrors(webs_t); // warning: implicit declaration of function `checkWebErrors'
...
int websAspRequest(webs_t wp, char_t *lpath)
{
	...
	if (strstr(lpath, websGetDefaultPage()) == NULL) {
    	if (checkWebErrors(wp)) {
		    fmtAlloc(&last, 256,
				"<script language='JavaScript'>"
				"top.window.location.replace('%s');</script>",
				websGetDefaultPage());
