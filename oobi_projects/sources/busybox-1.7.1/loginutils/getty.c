...
#ifdef CYCLADES
static const char opt_string[] ALIGN1 = "I:LH:f:hil:mt:wnp";
#else
...
static int bcode(const char *s)
{
	...
	if (errno) {
		bb_error_msg_and_die("bcode: %s/%d", s, value);
		return -1;
	}
...
#ifdef CYCLADES
static void getty_speed(struct options *op);
static void getty_speed(struct options *op)
{
	int fd, len;
	char buf[128];
	...
	parse_speeds(op, buf);  /* baud rate(s) */
