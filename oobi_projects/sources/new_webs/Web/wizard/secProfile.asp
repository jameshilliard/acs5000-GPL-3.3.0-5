...
<head>
	...
	function init()
	{
		if (1 == parent.menuReady && 1 == parent.topMenuReady && 1 == parent.controlsReady)
		{
			...
			page = parent.menu.getPageIndex('SRV', parent.menu.menuPages);
			parent.menu.selectItem('SRV');
...
<script type="text/javascript">
		if ((top.family_model == "ACS")||(top.family_model == "AVCS")) {
			...
			- The use of "Secured" profile disables all protocols except<br>\
			&nbsp;&nbsp;&nbsp;SSH v2 and HTTPS.<br>');
			if (<%get("system.device.kvmanalog");%> == 0) {
			document.write('\
...
<form name="configForm" method="POST" action="/goform/Dmf">
	<input type="hidden" name="system.req.action" value="">
	<input type="hidden" name="system.req.sid" value="<%get("_sid_");%>">
	<input type="hidden" name="urlOk" value="/wizard/secProfile.asp">
	<input type="hidden" name="urlError" value="/wizard/secProfile.asp">
   	<input type="hidden" name="request" value="<%write(ReqNum);%>">
	<!--*******************************************************************-->
	...
	<input type="hidden" name="system.security.profile.mode" value="0">
	<!--*******************************************************************-->
</form>
