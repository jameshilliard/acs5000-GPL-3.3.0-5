...
// this is necessary when sending traps to DSView because DSView will accept only the address it knows
void bindLocalAddress(int socket) {
	int rc = 0;
	char *localAddress = getenv("localAddress");
	//printf("bindLocalAddress\n");
	if (localAddress != NULL) {
		struct sockaddr_storage addr;
		const char *error;
		
		error = xinet_aton(localAddress,&addr);
		if (error == NULL) {
			rc = bind(socket, (struct sockaddr *) &addr,
				sizeof(addr));
