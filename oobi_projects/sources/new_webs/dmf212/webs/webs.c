 ...
 *
 * $Id: webs.c,v 1.2 2006/01/10 23:51:53 erik Exp $
 */
...
static char_t         *websHTTPHost;          /* Host from the "Host:" HTTP header */
...
#ifdef CYC_FIX
static int websGetReadOKTimeout(webs_t wp);
static void websSetTimeReadOK(webs_t wp);
#endif
 ...
 */
		...
		bfree(B_L, upperKey);
               ...
               /* Take advantage of the "Host:" header
                  if it exists. [EL] 3/24/2004  */
               if (gstrcmp(key,T("host")) == 0) {
                       /* Copy to the wp structure so that
                          it fits in with the other headers */
                       wp->host=bstrdup(B_L,value);
                       /* Copy to the global variable websHTTPHost
                         so that we can return its value */
                       websHTTPHost=bstrdup(B_L,value);
               }
...
// Oct/31/2005 Livio: Removed "hack" to redirect https based on 
// KVM security level. Now the redirection is based on the security profile.
...
{
	char_t	*h, *p, *c, *msgbuf, *urlbuf, *redirectFmt;
	int port;
	char tmp[256];
 ...
 */
	if (gstrstr(url, T("http://")) == NULL) {
		...
		if (wp->flags & WEBS_SECURE) { 
			...
			p = websSSLIpaddrUrl;
			port = 0;
//[RK]Aug/12/05 - Fixed ACS redirect problem
		} else if (strcmp(wp->protocol,"http") == 0 && seclevel > 0) {
			redirectFmt=T("https://%s/%s");
			p = websSSLIpaddrUrl;
			port = SSL_PORT;
		} else {
		...
		if ((h=websGetVar(wp, T("HTTP_HOST"), NULL)) == NULL) {
			h = p;
		} else {
			c = strchr(h, ':');
			if (port && port != 443 && c != NULL) {
				if (c) *c = 0;
				snprintf(tmp, sizeof(tmp), "%s:%d", h, port);
				if (c) *c = ':';
				h = tmp;
			}
		}
		fmtAlloc(&urlbuf, WEBS_MAX_URL + 80, redirectFmt, h, url);
		url = urlbuf;
	}
...
/******************************************************************************/
/*
 *    Return the Hostname in the HTTP header
 */
char_t *websGetHTTPHost(void)
{
	return(websHTTPHost);
}
...
static int websGetReadOKTimeout(webs_t wp)
{
	...
	return ((time(0) - wp->last_readOk) > 1);
}
#endif
